...
namespace GridIron {
    namespace Controls {
        ...
        {
            Status.value = PageStatus::INITIALIZING;
            int bytesread = 0;

            HtmlFile.value = std::string(codeBesideFilename);
            if (HtmlFile.value.empty()) {
                throw new std::invalid_argument("HtmlFile cannot be empty");
            }

            // make up an id until we parse and match up with one
            ID.value = std::string(Namespace).append("_").append(ControlType).append("_").append((std::string) HtmlFile.value); // default id = "_Page_" or "_Page_foobar.html"
            ...
            Status.value = PageStatus::INIT_READING;

            // open the front-page
            const std::string fullPagePath = PathToFile(codeBesideFilename);
            std::ifstream ifs(fullPagePath, std::ios_base::in);
            if (!ifs.is_open())
                throw GridException(101, std::string("unable to open front-end page: ").append(
                        fullPagePath).c_str());
            HtmlFilepath.value = fullPagePath;

            // add/link default registered variables
            _regvarsRO.insert({std::string(GRIDIRON_XHTML_NS).append(".FrontPage"),
                               std::make_shared<std::string_view>(HtmlFilepath.value)});
            _regvarsRO.insert({std::string(GRIDIRON_XHTML_NS).append(".CodeBesideFilename"),
                               std::make_shared<std::string_view>(HtmlFile.value)});
            ...
            // update status with another new property/RO status( does it work? )
            Status.value = PageStatus::INIT_PARSING;
            ...
            // update status with another new property/RO status( does it work? )
            Status.value = PageStatus::PARSED_READY;
        }
        ...
        bool
        Page::RegisterRWVariable(const std::string &name, std::string &data) {
            ...
            int pos = name.find_first_not_of(
                    "_-abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789."); // allow dots
            if (pos != std::string::npos) return false;

            // search through variables, make sure name isn't already registered
            for (std::map<const std::string, ReadOnlyProperty<std::string>>::iterator m = _regvarsRW.begin(); m != _regvarsRW.end();
            ++m) {
                if (m->first == name) return false;
            }

            // no existing var with same name found, register it
            class : public Property<std::string> { friend class Page; } p;
            _regvarsRW.insert({name, p});
            return true;
        }

        bool
        Page::RegisterROVariable(const std::string &name, const std::string& const data) {
            // NOTE: tags starting with __ should be system generated vars only, but we won't check
            ...
            // search through variables, make sure name isn't already registered
            for (std::map<const std::string, ReadOnlyProperty<std::string>>::iterator m = _regvarsRO.begin(); m != _regvarsRO.end(); ++m) {
                if (m->first == name) return false;
            }

            // no existing var with same name found, register it
            class : public ReadOnlyProperty<std::string> { friend class Page; } p;
            _regvarsRO.insert({name, p});
            return true;
        }
    };
};